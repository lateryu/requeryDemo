...
(function() {
  ...
  //大多数是内部函数生成可以应用的回调
  //集合中的每个元素,返回所需的结果——要么身份,任意调,匹配器属性,或属性访问器。
  ...
  // The cornerstone, an `each` implementation, aka `forEach`. 基石,“每一个”实现,又名“forEach”。
  // Handles raw objects in addition to array-likes. Treats all 除了类数组处理原始对象。对待所有稀疏的数组类,好像他们密集。
  ...
  //添加一些类型方法:isArguments、isFunction isString,isNumber,isDate,isRegExp,返回错误。
